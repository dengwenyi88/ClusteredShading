...
ShaderID Renderer::addShader(const char *fileName, const char **attributeNames, const int nAttributes, const char *extra, const uint flags){
	...
	if (file == NULL){
		String errStr("Couldn't load \"");
		errStr += fileName;
		errStr += "\"";
		ErrorMsg(errStr.c_str());
	} else {
#ifdef DEBUG
		char str[66];
		str[0] = '\n';
		...
		res = addShader(vs, gs, fs, cs, vsLine, gsLine, fsLine, csLine, header, extra, fileName, attributeNames, nAttributes, flags);
		delete [] shaderText;
	}
	return res;
}
