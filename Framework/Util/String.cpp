...
void String::sprintf(const char *formatStr, ... ){
	char buf[256];
	union {
		double farg;
		int    iarg;
...
String operator + (const String &string, const String &string2){
	...
	str = string;
	str.append(string2.c_str(), string2.getLength());
	return str;
}
...
//#define IMP_COMPARE(s0,op,s1) bool operator op (const s0 string, const s1 string2){ return (strcmp(string.c_str(), string2.c_str()) op 0); }
//
