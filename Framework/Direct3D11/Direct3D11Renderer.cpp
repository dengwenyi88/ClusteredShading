...
ShaderID Direct3D11Renderer::addShader(const char *vsText, const char *gsText, const char *fsText, const char *csText, const int vsLine, const int gsLine, const int fsLine, const int csLine,
									   const char *header, const char *extra, const char *fileName, const char **attributeNames, const int nAttributes, const uint flags)
{
	if (vsText == NULL && gsText == NULL && fsText == NULL && csText == NULL)
		return SHADER_NONE;

	if ((vsText != NULL || gsText != NULL || fsText != NULL) && csText != NULL) // No mixing of compute and graphics
		return SHADER_NONE;
	...
	if (vsText != NULL)
	{
		...
		if (SUCCEEDED(D3DCompile(shaderString.c_str(), shaderString.getLength(), fileName, NULL, NULL, "main", target, compileFlags, 0, &shaderBuf, &errorsBuf)))
		{
	...
	if (vsRefl)
	{
		...
		if (vsDesc.ConstantBuffers)
		{
			...
			memset(shader.vsDirty, 0, shader.nVSCBuffers * sizeof(bool));
		}
	}
	if (gsRefl)
	{
		...
		if (gsDesc.ConstantBuffers)
		{
			...
			memset(shader.gsDirty, 0, shader.nGSCBuffers * sizeof(bool));
		}
	}
	if (psRefl)
	{
		...
		if (psDesc.ConstantBuffers)
		{
			...
			memset(shader.psDirty, 0, shader.nPSCBuffers * sizeof(bool));
		}
	}
	if (csRefl)
	{
		...
		if (csDesc.ConstantBuffers)
		{
			...
			memset(shader.csDirty, 0, shader.nCSCBuffers * sizeof(bool));
		}
	}
...
void Direct3D11Renderer::drawPlain(const Primitives primitives, vec2 *vertices, const uint nVertices, const BlendStateID blendState, const DepthStateID depthState, const vec4 *color)
{
	...
	{
		plainShader = addShader(plainVS, NULL, plainPS, NULL, 0, 0, 0, 0);
...
void Direct3D11Renderer::drawTextured(const Primitives primitives, TexVertex *vertices, const uint nVertices, const TextureID texture, const SamplerStateID samplerState, const BlendStateID blendState, const DepthStateID depthState, const vec4 *color)
{
	...
	{
		texShader = addShader(texVS, NULL, texPS, NULL, 0, 0, 0, 0, texDefs);
...
ID3D11Resource *Direct3D11Renderer::getVBResource(const VertexBufferID buffer) const
{
	return vertexBuffers[buffer].vertexBuffer;
}

ID3D11Resource *Direct3D11Renderer::getIBResource(const IndexBufferID buffer) const
{
