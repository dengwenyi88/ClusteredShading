...
bool App::init()
{
	...
	if (!m_Map.loadObj("../Resource/Models/Corridor2/Map.obj")) return false;
...
bool App::load()
{
	...
	// Shaders
	if ((m_Clustered    = renderer->addShader("../Resource/Shader/Clustered.shd", DEFINE_STR(LIGHT_COUNT))) == SHADER_NONE) return false;
	if ((m_ShowClusters = renderer->addShader("../Resource/Shader/Clustered.shd", DEFINE_STR(LIGHT_COUNT) "#define SHOW_CLUSTERS\n")) == SHADER_NONE) return false;
	if ((m_PreZ         = renderer->addShader("../Resource/Shader/PreZ.shd")) == SHADER_NONE) return false;
	if ((m_FillBuffers  = renderer->addShader("../Resource/Shader/FillBuffers.shd")) == SHADER_NONE) return false;
	if ((m_CreateMask   = renderer->addShader("../Resource/Shader/CreateMask.shd")) == SHADER_NONE) return false;
	if ((m_Ambient[0]   = renderer->addShader("../Resource/Shader/Ambient.shd", def)) == SHADER_NONE) return false;
	if ((m_Lighting[0]  = renderer->addShader("../Resource/Shader/Lighting.shd", def)) == SHADER_NONE) return false;
	strcat(def, "#define SINGLE_SAMPLE\n");
	if ((m_Ambient[1]   = renderer->addShader("../Resource/Shader/Ambient.shd", def)) == SHADER_NONE) return false;
	if ((m_Lighting[1]  = renderer->addShader("../Resource/Shader/Lighting.shd", def)) == SHADER_NONE) return false;
	if ((m_LightBlob[0] = renderer->addShader("../Resource/Shader/LightBlob.shd")) == SHADER_NONE) return false;
	if ((m_LightBlob[1] = renderer->addShader("../Resource/Shader/LightBlob.shd", "#define DEFERRED\n")) == SHADER_NONE) return false;
	...
	// Textures
	if ((m_Base[0] = renderer->addTexture  ("../Resource/Textures/Tx_wood_brown_shelf_small.dds",                    true, m_TrilinearAniso, SRGB)) == TEXTURE_NONE) return false;
	if ((m_Bump[0] = renderer->addNormalMap("../Resource/Textures/Tx_wood_brown_shelf_smallBump.dds", FORMAT_RGBA8S, true, m_TrilinearAniso)) == TEXTURE_NONE) return false;

	if ((m_Base[1] = renderer->addTexture  ("../Resource/Textures/wallpaper20.dds",                    true, m_TrilinearAniso, SRGB)) == TEXTURE_NONE) return false;
	if ((m_Bump[1] = renderer->addNormalMap("../Resource/Textures/wallpaper20Bump.dds", FORMAT_RGBA8S, true, m_TrilinearAniso)) == TEXTURE_NONE) return false;

	if ((m_Base[2] = renderer->addTexture  ("../Resource/Textures/floor_wood_4.dds",                    true, m_TrilinearAniso, SRGB)) == TEXTURE_NONE) return false;
	if ((m_Bump[2] = renderer->addNormalMap("../Resource/Textures/floor_wood_4Bump.dds", FORMAT_RGBA8S, true, m_TrilinearAniso)) == TEXTURE_NONE) return false;

	if ((m_Base[3] = renderer->addTexture  ("../Resource/Textures/floor_wood_3.dds",                    true, m_TrilinearAniso, SRGB)) == TEXTURE_NONE) return false;
	if ((m_Bump[3] = renderer->addNormalMap("../Resource/Textures/floor_wood_3Bump.dds", FORMAT_RGBA8S, true, m_TrilinearAniso)) == TEXTURE_NONE) return false;

	if ((m_Base[4] = renderer->addTexture  ("../Resource/Textures/light2.dds",                    true, m_TrilinearAniso, SRGB)) == TEXTURE_NONE) return false;
	if ((m_Bump[4] = renderer->addNormalMap("../Resource/Textures/light2Bump.dds", FORMAT_RGBA8S, true, m_TrilinearAniso)) == TEXTURE_NONE) return false;
